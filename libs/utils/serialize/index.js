...
const serialize = async (msg, client) => {
  ...
  if (msg.message[m.type]?.contextInfo?.quotedMessage) {
    ...
    m.quoted.delete = () =>
      client.sendMessage(m.from, { delete: m.quoted.key });
    m.quoted.download = (type = "stream") =>
  ...
  m.replyWithMentions = (text, mentions, quoted) => {
    return (
      ...
      client.sendMessage(
        m.from,
        { text: text.toString().trim(), mentions },
        { quoted: quoted || msg }
      )
    );
  };
  ...
  m.replyLocation = (location) => {
    return (
      !m.isSelf && client.sendMessage(m.from, { location }, { quoted: msg })
    );
  };
  m.replyTemplateButton = (text, templateButtons, title, footer) => {
    return (
      ...
      client.sendMessage(
        m.from,
        { text, footer, templateButtons, title, viewOnce: false },
        { quoted: msg }
      )
    );
  };
  m.replyButton = (text, buttons, title, footer) => {
    return (
      ...
      client.sendMessage(
        m.from,
        { text, footer, buttons, title, viewOnce: false },
        { quoted: msg }
      )
    );
  };
  m.replyList = (text, buttonText, sections, title, footer) => {
    return (
      ...
      client.sendMessage(
        m.from,
        { text, buttonText, sections, title, footer, viewOnce: false },
        { quoted: msg }
      )
    );
  };
