...
module.exports = async (client, { messages, type }) => {
  ...
  if (getCommand) {
    const command_log = [
      chalk.whiteBright("├"),
      chalk.keyword("aqua")(`[ ${msg.isGroup ? " GROUP " : "PRIVATE"} ]`),
      msg.body.substr(0, 50).replace(/\n/g, ""),
      chalk.greenBright("from"),
      chalk.yellow(msg.senderNumber),
      chalk.whiteBright("Tag Name :"),
      chalk.greenBright(msg.pushName),
    ];
    if (msg.isGroup) {
    ...
    if (
      getCommand.ownerOnly &&
      !metaData.developer.number.includes(msg.senderNumber)
    ) {
    ...
    if (getCommand.cooldown) {
      ...
      if (
        ...
        return msg.reply(
          i18n.__("message.cooldown", {
            cooldown: Math.round(duration.asSeconds()),
          })
        );
      }
      if (
